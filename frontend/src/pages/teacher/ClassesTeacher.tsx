...
import { fetchClasses, fetchTeacherInvites, updateInviteStatus, Invite } from "../../util/teacher/httpTeacher";
    ...
    <div className="flex flex-col items-center w-full space-y-10">
      ...
      <div className="w-full px-10">
        ...
        {!isLoading && !isError && classes && classes.length > 0 ? (
          <div>
            <h2 className="text-2xl font-bold mb-4">Mijn Klassen</h2>
                ...
                {classes.map((classItem) => (
                  <tr key={classItem.id}>
                    <td className="border px-4 py-2">
                      <PrimaryButton onClick={() => navigate(`/teacher/classes/${classItem.id}`)}>
                        Beheer
                      </PrimaryButton>
                    </td>
      ...
      <div className="w-full px-10">
        ...
        {!isInvitesLoading && !isInvitesError && invites && invites.length > 0 && (
          <div>
                ...
                {invites.map((invite) => (
                  <tr key={invite.inviteId}>
                    ...
                    <td className="border px-2 py-1">
                      {invite.otherTeacher.user.firstName} {invite.otherTeacher.user.lastName}
                    </td>
                    <td className="border px-2 py-1">{invite.status}</td>
                    <td className="border px-2 py-1 flex gap-2">
                      {invite.status === "PENDING" && (
                        <>
                          <PrimaryButton onClick={() => handleAccept(invite.inviteId)}>
                            Accepteer
                          </PrimaryButton>
                          <PrimaryButton onClick={() => handleDecline(invite.inviteId)}>
                            Weiger
                          </PrimaryButton>
                        </>
                      )}
