...
function extractSortableFields(input: string): string[] {
    return input?.split(",").filter(field => allowedSortFields.includes(field)) || ["deadline"];
}

export const getStudentAssignments = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        if (!req.user) {
        ...
        res.status(500).json({ error: "Something went wrong while requesting the assignments" });
    }
};
