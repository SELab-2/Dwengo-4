...
const registerUser = async (req: AuthenticatedRequest, res: Response, role: Role): Promise<void> => {
    ...
    try {
        ...
        if (error instanceof z.ZodError) {
            res.status(400).json({
                message: error.errors.map((err: ZodIssue): string => err.message).join(", "),
            });
            return;
        } else {
...
// @access  Public
export const registerTeacher = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
...
// @access  Public
export const registerStudent = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
