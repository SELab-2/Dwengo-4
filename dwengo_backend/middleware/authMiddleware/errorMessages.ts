export const userNotFoundMessage = "User not found.";
export const teacherNotFoundMessage = "Teacher not found.";
export const studentNotFoundMessage = "Student not found.";
export const neitherTeacherNorStudentMessage =
  "User is neither a teacher nor a student.";
export const invalidTokenMessage = "Invalid authorization token.";
export const noTokenProvidedMessage = "No authorization token.";
