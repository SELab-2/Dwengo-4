...
export default class joinRequestService {
  ...
  static async createValidJoinRequest(
    ...
    try {
      const classroom: ClassWithLinks =
        await this.validateClassExists(joinCode);
      ...
      const existingRequest: JoinRequest | null =
  ...
  static async approveRequestAndAddStudentToClass(
    ...
    try {
      ...
      this.handleError(
        error,
        `Error approving join request ${requestId} for class ${classId}`
      );
      throw error;
    }
  }
  ...
  static async denyJoinRequest(
    ...
    try {
      ...
      this.handleError(
        error,
        `Error denying join request ${requestId} for class ${classId}`
      );
      throw error;
    }
  }
  ...
  static async getJoinRequestsByClass(
    ...
    try {
      ...
      return await prisma.joinRequest
        .findMany({
          where: { classId },
          include: {
            student: {
      ...
      console.error(
        `Error fetching join requests for class ${classId}:`,
        error
      );
      throw new Error(
