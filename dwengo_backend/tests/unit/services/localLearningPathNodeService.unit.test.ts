...
vi.mock("../../../config/prisma", async () => {
  ...
  const prisma = mockDeep<typeof import("../../../config/prisma").default>();
...
beforeEach(() => {
  ...
  (prisma.learningPath.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
    id: pathId,
    creatorId: teacherId,
  });
...
describe("LocalLearningPathNodeService", () => {
  it("creëert een lokale node", async () => {
    ...
    expect(mockTx.learningPathNode.create).toHaveBeenCalled();
  ...
  it("creëert een externe node (Dwengo)", async () => {
    (dwengoAPI.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: { someMeta: true } });
  ...
  it("verwijdert een node met node count update", async () => {
    (prisma.learningPathNode.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
      nodeId,
    ...
    expect(mockTx.learningPathNode.delete).toHaveBeenCalledWith({ where: { nodeId } });
    expect(mockTx.learningPath.update).toHaveBeenCalledWith({
      where: { id: pathId },
      data: { num_nodes: 0 },
    });
