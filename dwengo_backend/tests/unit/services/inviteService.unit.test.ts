...
describe("inviteService", () => {
  ...
  describe("createInvite", () => {
    it("throws if class not found", async () => {
      (classService.getClassById as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    ...
    it("throws if classTeacher is not teacher of class", async () => {
      (classService.getClassById as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 1 } as any);
    ...
    it("throws if there is an existing pending invite", async () => {
      ...
      (prisma.invite.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(mockInvite);
  ...
  describe("getPendingInvitesForClass", () => {
    ...
    it("returns invites for class", async () => {
      (classService.isTeacherOfClass as ReturnType<typeof vi.fn>).mockResolvedValue(true);
      (prisma.invite.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([mockInvite]);
  ...
  describe("acceptInviteAndJoinClass", () => {
    ...
    it("accepts invite and creates classTeacher", async () => {
      (prisma.invite.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(mockInvite);
      (prisma.$transaction as ReturnType<typeof vi.fn>).mockResolvedValue([mockInvite]);
  ...
  describe("deleteInvite", () => {
    ...
    it("deletes invite", async () => {
      (classService.isTeacherOfClass as ReturnType<typeof vi.fn>).mockResolvedValue(true);
      (prisma.invite.delete as ReturnType<typeof vi.fn>).mockResolvedValue(mockInvite);
