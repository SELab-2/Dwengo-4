...
describe('ReferenceValidationService', () => {
  ...
  describe('validateLocalLearningObject', () => {
    it('should resolve if learning object exists', async () => {
      (prisma.learningObject.findUnique as any).mockResolvedValue({ id: 'abc123' });
      await expect(ReferenceValidationService.validateLocalLearningObject('abc123')).resolves.toBeUndefined();
    });

    it('should throw error if learning object does not exist', async () => {
      (prisma.learningObject.findUnique as any).mockResolvedValue(null);
      await expect(ReferenceValidationService.validateLocalLearningObject('not-found')).rejects.toThrow(
        "Lokaal leerobject 'not-found' niet gevonden."
      );
    });
  });
  ...
  describe('validateDwengoLearningObject', () => {
    it('should resolve if valid data received from Dwengo API', async () => {
      (dwengoAPI.get as any).mockResolvedValue({ data: { title: 'object title' } });
      await expect(
    ...
    it('should throw error if Dwengo API returns 404', async () => {
      ...
      (dwengoAPI.get as any).mockRejectedValue(error);
    ...
    it('should throw error if Dwengo API throws other error', async () => {
      (dwengoAPI.get as any).mockRejectedValue(new Error('Connection error'));
      await expect(
        ReferenceValidationService.validateDwengoLearningObject('abc', 'nl', 1)
    ...
    it('should throw error if response is missing data', async () => {
      (dwengoAPI.get as any).mockResolvedValue({ data: null });
      await expect(
        ReferenceValidationService.validateDwengoLearningObject('abc', 'nl', 1)
      ).rejects.toThrow(
        'Dwengo leerobject hruid=abc,language=nl,version=1 => geen data ontvangen.'
      );
    });
  });
  ...
  describe('validateLocalLearningPath', () => {
    it('should resolve if learning path exists', async () => {
      (prisma.learningPath.findUnique as any).mockResolvedValue({ id: 'path123' });
      await expect(ReferenceValidationService.validateLocalLearningPath('path123')).resolves.toBeUndefined();
    });

    it('should throw error if learning path does not exist', async () => {
      (prisma.learningPath.findUnique as any).mockResolvedValue(null);
      await expect(ReferenceValidationService.validateLocalLearningPath('not-found')).rejects.toThrow(
        "Lokaal leerpad 'not-found' niet gevonden."
      );
    });
  });
  ...
  describe('validateDwengoLearningPath', () => {
    it('should resolve if Dwengo API returns array of results', async () => {
      (dwengoAPI.get as any).mockResolvedValue({ data: [{ id: 1 }] });
      await expect(
    ...
    it('should throw error if Dwengo API returns empty array', async () => {
      (dwengoAPI.get as any).mockResolvedValue({ data: [] });
      await expect(
        ReferenceValidationService.validateDwengoLearningPath('pad123', 'nl')
      ).rejects.toThrow(
        'Dwengo leerpad (hruid=pad123, language=nl) niet gevonden (lege array).'
      );
    });

    it('should throw error on Dwengo API failure', async () => {
      (dwengoAPI.get as any).mockRejectedValue(new Error('API failure'));
      await expect(
        ReferenceValidationService.validateDwengoLearningPath('pad123', 'nl')
