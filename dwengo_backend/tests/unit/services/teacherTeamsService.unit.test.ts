...
describe("teacherTeamsService", () => {
  ...
  describe("createTeamsInAssignment", () => {
    it("throws error if assignment is not linked to class", async () => {
      (
        prisma.classAssignment.findMany as ReturnType<typeof vi.fn>
      ).mockResolvedValue([]);

      await expect(createTeamsInAssignment(1, 2, [])).rejects.toThrow(
        "Assignment not found or not linked to any class.",
      );
    });
