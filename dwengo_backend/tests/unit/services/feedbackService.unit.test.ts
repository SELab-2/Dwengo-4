...
describe('FeedbackService', () => {
  ...
  describe('hasAssignmentRights', () => {
    it('retourneert true als teacher rechten heeft', async () => {
      (prisma.teacher.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ userId: 1 });
  ...
  describe('getAllFeedbackForEvaluation', () => {
    it('retourneert feedbacklijst als rechten ok zijn', async () => {
      (prisma.teacher.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ userId: 1 });
      (prisma.feedback.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: 1 }, { id: 2 }]);
  ...
  describe('createFeedback', () => {
    it('maakt feedback aan als alles ok is', async () => {
      ...
      (prisma.assignment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    ...
    it('gooit error als deadline in toekomst ligt', async () => {
      (prisma.teacher.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ userId: 1 });
      (prisma.assignment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 99 });
  ...
  describe('deleteFeedbackForSubmission', () => {
    it('verwijdert feedback bij geldige rechten', async () => {
      (prisma.teacher.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ userId: 1 });
      (prisma.feedback.delete as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 99 });
