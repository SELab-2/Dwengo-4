...
describe('joinRequestService', () => {
  ...
  describe('createJoinRequest', () => {
    it('creates a join request', async () => {
      ...
      (prisma.joinRequest.create as ReturnType<typeof vi.fn>).mockResolvedValue(expected);
  ...
  describe('createValidJoinRequest', () => {
    it('throws if student is already in class', async () => {
      (classService.getClassByJoinCode as ReturnType<typeof vi.fn>).mockResolvedValue({ id: classId });
      (classService.isStudentInClass as ReturnType<typeof vi.fn>).mockResolvedValue(true);
    ...
    it('throws if pending request already exists', async () => {
      (classService.getClassByJoinCode as ReturnType<typeof vi.fn>).mockResolvedValue({ id: classId });
      (classService.isStudentInClass as ReturnType<typeof vi.fn>).mockResolvedValue(false);
      (prisma.joinRequest.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({});
    ...
    it('creates valid request successfully', async () => {
      ...
      (classService.getClassByJoinCode as ReturnType<typeof vi.fn>).mockResolvedValue({ id: classId });
      (classService.isStudentInClass as ReturnType<typeof vi.fn>).mockResolvedValue(false);
      (prisma.joinRequest.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
      (prisma.joinRequest.create as ReturnType<typeof vi.fn>).mockResolvedValue(expected);
  ...
  describe('approveRequestAndAddStudentToClass', () => {
    it('updates request to approved and adds student to class', async () => {
      ...
      (classService.isTeacherOfClass as ReturnType<typeof vi.fn>).mockResolvedValue(true);
      (prisma.joinRequest.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(req);
      (prisma.joinRequest.update as ReturnType<typeof vi.fn>).mockResolvedValue({ ...req, status: JoinRequestStatus.APPROVED });
      (classService.addStudentToClass as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
  ...
  describe('denyJoinRequest', () => {
    it('updates join request to denied', async () => {
      ...
      (classService.isTeacherOfClass as ReturnType<typeof vi.fn>).mockResolvedValue(true);
      (prisma.joinRequest.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(req);
      (prisma.joinRequest.update as ReturnType<typeof vi.fn>).mockResolvedValue({ ...req, status: JoinRequestStatus.DENIED });
  ...
  describe('getJoinRequestsByClass', () => {
    it('throws error if teacher not part of class', async () => {
      (classService.isTeacherOfClass as ReturnType<typeof vi.fn>).mockResolvedValue(false);
    ...
    it('returns all join requests including student info', async () => {
      (classService.isTeacherOfClass as ReturnType<typeof vi.fn>).mockResolvedValue(true);
      (prisma.joinRequest.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
        {
          requestId: 3,
          studentId,
          classId,
