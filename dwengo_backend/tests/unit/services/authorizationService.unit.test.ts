...
import { isAuthorized, canUpdateOrDelete } from "../../../services/authorizationService";
...
vi.mock("../../../config/prisma", () => ({
  default: {
    ...
    classStudent: { findFirst: vi.fn() },
    classAssignment: { findFirst: vi.fn() },
  },
}));

// Helper voor volledige user mock
...
describe("authorizationService", () => {
  ...
  describe("isAuthorized()", () => {
    it("should return true for ADMIN regardless of role/classId", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.ADMIN }));
      const result = await isAuthorized(1, Role.TEACHER, 123);
      expect(result).toBe(true);
    });

    it("should return false if user has wrong role", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.STUDENT }));
    ...
    it("should throw error if user not found", async () => {
      ...
      await expect(isAuthorized(1, Role.TEACHER)).rejects.toThrow("User not found");
    });

    it("should return true for TEACHER without class check", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.TEACHER }));
    ...
    it("should return false if TEACHER not linked to classId", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.TEACHER }));
      (prisma.classTeacher.findFirst as any).mockResolvedValue(null);
    ...
    it("should return true if TEACHER linked to classId", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.TEACHER }));
      (prisma.classTeacher.findFirst as any).mockResolvedValue({ id: 1 });
      const result = await isAuthorized(3, Role.TEACHER, 99);
      expect(result).toBe(true);
    });
    ...
    it("should return true if STUDENT is enrolled in classId", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.STUDENT }));
      (prisma.classStudent.findFirst as any).mockResolvedValue({ id: 10 });
      const result = await isAuthorized(4, Role.STUDENT, 321);
      expect(result).toBe(true);
    });
  });
  ...
  describe("canUpdateOrDelete()", () => {
    ...
    it("should return false if no assignments found for teacher’s classes", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.TEACHER }));
      (prisma.classTeacher.findMany as any).mockResolvedValue([{ classId: 1 }]);
      (prisma.classAssignment.findFirst as any).mockResolvedValue(null);
    ...
    it("should return true if assignment is linked to teacher’s class", async () => {
      (prisma.user.findUnique as any).mockResolvedValue(mockUser({ role: Role.TEACHER }));
      (prisma.classTeacher.findMany as any).mockResolvedValue([{ classId: 1 }, { classId: 2 }]);
      (prisma.classAssignment.findFirst as any).mockResolvedValue({ assignmentId: 100 });
