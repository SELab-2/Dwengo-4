...
describe("authorizationService", () => {
  ...
  describe("isAuthorized()", () => {
    ...
    it("should return false if TEACHER not linked to classId", async () => {
      ...
      (prisma.classTeacher.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    ...
    it("should return true if TEACHER linked to classId", async () => {
      ...
      (prisma.classTeacher.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 1 });
      const result = await isAuthorized(3, Role.TEACHER, 99);
      expect(result).toBe(true);
    });
    ...
    it("should return true if STUDENT is enrolled in classId", async () => {
      ...
      (prisma.classStudent.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 10 });
      const result = await isAuthorized(4, Role.STUDENT, 321);
      expect(result).toBe(true);
    });
  });
  ...
  describe("canUpdateOrDelete()", () => {
    ...
    it("should return false if no assignments found for teacher’s classes", async () => {
      ...
      (prisma.classTeacher.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ classId: 1 }]);
      (prisma.classAssignment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
    ...
    it("should return true if assignment is linked to teacher’s class", async () => {
      ...
      (prisma.classTeacher.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
        { classId: 1 },
        { classId: 2 },
      ]);
      (prisma.classAssignment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({
        assignmentId: 100,
      });
