...
describe("StudentService", () => {
  ...
  describe("findStudentById", () => {
    it("retourneert student met gekoppelde user als ID bestaat", async () => {
      ...
      (
        prisma.student.findUniqueOrThrow as ReturnType<typeof vi.fn>
    ...
    it("gooit error door als student niet gevonden wordt", async () => {
      ...
      await expect(StudentService.findStudentById(999)).rejects.toThrow(
        "Not found",
      );
    });
  });
  ...
  describe("getStudentsByTeamAssignment", () => {
    ...
    it("gooit error als prisma faalt", async () => {
      (prisma.student.findMany as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error("Query failed"),
      );

      await expect(
        StudentService.getStudentsByTeamAssignment(1, 2),
