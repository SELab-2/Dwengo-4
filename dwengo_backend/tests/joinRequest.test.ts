...
import {Class, ClassStudent, JoinRequest, JoinRequestStatus, Student, Teacher, User} from '@prisma/client';
import {
    addStudentToClass,
    addTeacherToClass,
    createClass,
...
async function sendPostWithJoinCode(): Promise<{status: number, body: any}> {
...
async function verifyStudentDidntJoin(joinRequest: JoinRequest): Promise<void> {
    ...
    const classStudent: ClassStudent | null = await prisma.classStudent.findUnique({
        where: {
            studentId_classId: {
...
async function patchAndApproveJoinRequest(joinRequest: JoinRequest): Promise<{ status: number, body: any }> {
    const { status, body } = await request(app)
        .patch(`/teacher/classes/${classroom.id}/join-requests/${joinRequest.requestId}`)
...
describe('join request tests', async(): Promise<void> => {
    ...
    describe('[POST] /student/classes/join', async(): Promise<void> => {
        it('should respond with a `201` status code and return the created join request', async(): Promise<void> => {
            ...
            const { status, body } = await sendPostWithJoinCode();
        ...
        it('should respond with a `201` status code and return the created join request, even when a previous join request was denied before', async() => {
            ...
            let joinRequest: JoinRequest = await createJoinRequest(studentUser1.id, classroom.id);
            ...
            await prisma.joinRequest.findMany().then((joinRequests: JoinRequest[]): void => {
    ...
    describe('[PATCH] /teacher/classes/:classId/join-requests/:requestId', async(): Promise<void> => {
        ...
        it('should respond with a `200` status code and an updated join request when the action is `approve`', async(): Promise<void> => {
            ...
            const { status, body } = await patchAndApproveJoinRequest(joinRequest);
        ...
        it('should respond with a `400` status code when the action is neither `approve` or `deny`', async(): Promise<void> => {
            ...
            await verifyStudentDidntJoin(joinRequest);
        });
        it('should respond with a `404` status code when the join request does not exist', async(): Promise<void> => {
            ...
            expect(status).toBe(404);
            expect(body.error).toBe(`Error approving join request ${joinRequest.requestId} for class ${classroom.id}: Join request ${joinRequest.requestId} for class ${classroom.id} not found/not pending.`);
        });
        ...
        it('should respond with a `403` status code when the teacher is not allowed to approve/deny the request', async(): Promise<void> => {
            ...
            const teacherUser2: User & { teacher: Teacher, token: string } = await createTeacher("bleep", "blop", "bleep.blop@gmail.com");
            ...
            expect(body.error).toBe(`Error approving join request ${joinRequest.requestId} for class ${classroom.id}: Teacher ${teacherUser2.id} is not a teacher of class ${classroom.id}`);
            // verify that the join request was not updated
            await verifyStudentDidntJoin(joinRequest);
        });
    });
    describe('[GET] /teacher/classes/:classId/join-requests', async(): Promise<void> => {
